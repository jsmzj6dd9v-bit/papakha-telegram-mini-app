...
const route = async (request, env) => {
  ...
  if (path.startsWith("/api/admin/")) {
    ...
    if (method === "GET" && path === "/api/admin/dashboard") {
      ...
      return json({ ok: true, dashboard: { ...dashboard, services: { rates: ratesState, telegram: env.TELEGRAM_BOT_TOKEN ? "configured" : "unavailable" } } });
    }
  ...
  if (method === "POST" && path === "/api/deals") {
    ...
    const result = await createDeal(env, { user, requestId, input, quote, rates });
